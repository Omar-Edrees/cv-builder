import { createClient } from '@supabase/supabase-js';

// Service-role client: bypasses Row Level Security
function adminClient() {
  return createClient(
    process.env.SUPABASE_URL,
    process.env.SUPABASE_SERVICE_ROLE_KEY
  );
}

// Verify the caller is an admin by checking their JWT against the profiles table
async function verifyAdmin(authHeader) {
  if (!authHeader?.startsWith('Bearer ')) return null;
  const token = authHeader.slice(7);

  // Decode the JWT payload (public, unsigned portion) to extract sub (user id)
  const [, payload] = token.split('.');
  if (!payload) return null;
  const { sub } = JSON.parse(Buffer.from(payload, 'base64url').toString());
  if (!sub) return null;

  const sb = adminClient();
  const { data: profile } = await sb.from('profiles').select('role').eq('id', sub).single();
  return profile?.role === 'admin' ? sub : null;
}

export default async function handler(req, res) {
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'GET, PATCH, DELETE, OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization');
  if (req.method === 'OPTIONS') return res.status(200).end();

  const adminId = await verifyAdmin(req.headers.authorization).catch(() => null);
  if (!adminId) return res.status(403).json({ error: 'Forbidden' });

  const sb = adminClient();

  // ── GET /api/admin/users — list all users ──────────────────────────────────
  if (req.method === 'GET') {
    // Fetch all auth users via Admin API
    const { data: { users: authUsers }, error } = await sb.auth.admin.listUsers();
    if (error) return res.status(500).json({ error: error.message });

    // Fetch profiles (roles)
    const { data: profiles } = await sb.from('profiles').select('id, role');
    const profileMap = Object.fromEntries((profiles || []).map(p => [p.id, p]));

    // Fetch which users have saved data
    const { data: dataRows } = await sb.from('user_data').select('user_id');
    const hasDataSet = new Set((dataRows || []).map(r => r.user_id));

    const users = authUsers.map(u => ({
      id:              u.id,
      email:           u.email,
      created_at:      u.created_at,
      last_sign_in_at: u.last_sign_in_at,
      role:            profileMap[u.id]?.role || 'user',
      has_data:        hasDataSet.has(u.id)
    }));

    return res.status(200).json({ users });
  }

  // ── PATCH /api/admin/users — update role ───────────────────────────────────
  if (req.method === 'PATCH') {
    const { userId, role } = req.body || {};
    if (!userId || !['user', 'admin'].includes(role)) {
      return res.status(400).json({ error: 'Invalid payload' });
    }
    const { error } = await sb.from('profiles')
      .upsert({ id: userId, role }, { onConflict: 'id' });
    if (error) return res.status(500).json({ error: error.message });
    return res.status(200).json({ ok: true });
  }

  // ── DELETE /api/admin/users — delete user ──────────────────────────────────
  if (req.method === 'DELETE') {
    const { userId } = req.body || {};
    if (!userId) return res.status(400).json({ error: 'userId required' });

    // Delete from auth (cascades to user_data and profiles via FK)
    const { error } = await sb.auth.admin.deleteUser(userId);
    if (error) return res.status(500).json({ error: error.message });
    return res.status(200).json({ ok: true });
  }

  return res.status(405).json({ error: 'Method not allowed' });
}
